...
public class MemberDaoImpl implements MemberDao{
 ...
 DBConnectionPool connectionPool;
  ...
  public void add(Member member) {
      try (Connection con = connectionPool.getConnection();
        PreparedStatement pstmt = con.prepareStatement(
            "insert into members(email,name,password,tel,creditNo,creditDate) values(?,?,sha2(?,256),?,?,?)")) {
  ...
  public List<Member> findAll() {
    try (Connection con = connectionPool.getConnection();
        PreparedStatement pstmt = con.prepareStatement(
        "select member_no, email, name, tel, created_date from members");
  ...
  public Member findBy(int no) {
    try (Connection con = connectionPool.getConnection();
    PreparedStatement pstmt = con.prepareStatement(
        "select member_no, email, name, tel, created_date from members where member_no=?")){
      pstmt.setInt(1, no);
  ...
  String sql = null;
    if (member.getPassword().length() == 0) {
      sql = "update members set email=?, name=?, tel=? where member_no=?";
    } else {
      sql = "update members set email=?, name=?, tel=?, password=sha2(?,256) where member_no=?";
    }
